...
object TimeUsage extends TimeUsageInterface {
  ...
  def row(line: List[String]): Row = {
    val key: String = line.head
    val values: List[Double] = line.tail.map(_.toDouble)
    Row.fromSeq(key::values)
  }
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    val primaryCols: List[String] =  List("t01", "t03", "t11", "t1801", "t1803")
    val workingCols: List[String] = List("t05", "t1805")
    val otherCols: List[String] = List("t02", "t04", "t06", "t07", "t08", "t09", "t10", "t12", "t13", "t14", "t15", "t16", "t18" )

    def columnsStartWith(toMatch: List[String], col: String): Boolean = {
      toMatch.map(col.startsWith).reduce(_||_)
    }

    val primary: List[Column] = columnNames.filter(str => columnsStartWith(primaryCols, str)).map(new Column(_))
    val working: List[Column] = columnNames.filter(str => columnsStartWith(workingCols, str)).map(new Column(_))
    val other: List[Column] = columnNames.filter(str => {
        columnsStartWith(otherCols, str) && !columnsStartWith(primaryCols, str) && !columnsStartWith(workingCols, str)
      }).map(new Column(_))
  ...
  def timeUsageGroupedTyped(summed: Dataset[TimeUsageRow]): Dataset[TimeUsageRow] = {
    ...
    def round1(x: Double) = math.round(x * 10).toDouble / 10
