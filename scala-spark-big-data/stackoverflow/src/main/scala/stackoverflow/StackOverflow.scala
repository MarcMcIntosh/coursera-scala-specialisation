...
class StackOverflow extends StackOverflowInterface with Serializable {
  ...
  def scoredPostings(grouped: RDD[(QID, Iterable[(Question, Answer)])]): RDD[(Question, HighScore)] = {
    ...
    /* grouped.mapValues {
      value => {
        val question = value.head._1
        val answers = value.map(_._2).toArray
  ...
  def vectorPostings(scored: RDD[(Question, HighScore)]): RDD[(LangIndex, HighScore)] = {
    ...
    scored
      .map(score => {
        val maybeScore = firstLangInTag(score._1.tags, langs).getOrElse(-1)
        (maybeScore * langSpread, score._2)
      }).filter(_._1 >= 0).cache()
  }
  ...
  def clusterResults(means: Array[(Int, Int)], vectors: RDD[(LangIndex, HighScore)]): Array[(String, Double, Int, Int)] = {
    ...
    val median = closestGrouped.mapValues { vs =>
      ...
      val langKeys = grouped.maxBy(_._2)._1
      val langLabel: String   = langs(langKeys) // most common language in the cluster
      val langPercent: Double = grouped(langKeys) * 100.0d / vs.size // percent of the questions in the most common language
