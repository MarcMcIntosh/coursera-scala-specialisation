...
object WikipediaRanking extends WikipediaRankingInterface {
  ...
  val conf: SparkConf = new SparkConf().setMaster("local").setAppName("lang-wiki-data")
  ...
  def rankLangs(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] = {
    ...
    // action
    // val sorted = occurrences.sortBy(_._2)
    val sorted = occurrences.sortBy {
      case (_, count) => count
    }
    sorted.reverse
  }
  ...
  def makeIndex(langs: List[String], rdd: RDD[WikipediaArticle]): RDD[(String, Iterable[WikipediaArticle])] = {
    val occurrences = rdd.flatMap { article =>
      val words = article.text.split(" ")
      val mentioned = langs.filter(words.contains)
      mentioned.map(lang => (lang, article))
    }
    occurrences.groupByKey
  }
  ...
  def rankLangsReduceByKey(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] = {
    rdd.flatMap(article => {
      val words = article.text.split(" ")
      langs.filter(words.contains).map(lang => (lang, 1))
    }).reduceByKey(_ + _)
      .sortBy(_._2)
      .collect()
      .toList
