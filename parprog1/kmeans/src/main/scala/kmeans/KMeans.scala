...
class KMeans {
  ...
  def classify(points: GenSeq[Point], means: GenSeq[Point]): GenMap[Point, GenSeq[Point]] = {
    val pointsDistanceToMean = points.par.groupBy(findClosest(_, means))

    means.par.map(mean => (mean, pointsDistanceToMean.getOrElse(mean, GenSeq()))).toMap
  }
  ...
  final def kMeans(points: GenSeq[Point], means: GenSeq[Point], eta: Double): GenSeq[Point] = {
    ...
    val cov = converged(eta)(means, newMeans)

    if (!cov) kMeans(points, newMeans, eta) else newMeans // your implementation need to be tail recursive
  }
}
